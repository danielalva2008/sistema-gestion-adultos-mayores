...
$stmt = $pdo->prepare("SELECT * FROM incidentes WHERE id_incidente = ?");
$stmt->execute([$id]);
...
$residentes = $pdo->query(
    "SELECT id_residente, codigo_residente, CONCAT(nombres, ' ', apellidos) AS nombre
     FROM residentes ORDER BY apellidos, nombres"
)->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id_residente <= 0) $errores[] = 'Debe seleccionar un residente.';
    if ($id_tipo <= 0) $errores[] = 'Debe seleccionar un tipo de incidente.';
    if ($fecha_hora === '') $errores[] = 'La fecha y hora son obligatorias.';
    ...
    if ($fecha_cierre !== null && $fecha_cierre !== '') {
        ...
        if (strlen($fecha_cierre) === 16) $fecha_cierre .= ':00';
    }
        ...
        try {
            $stmt = $pdo->prepare(
                "UPDATE incidentes SET
                    id_residente = ?, id_tipo_incidente = ?, id_personal_reporta = ?,
                    fecha_hora = ?, lugar = ?, descripcion = ?, accion_realizada = ?,
                    requiere_seguimiento = ?, estado = ?, fecha_cierre = ?
                 WHERE id_incidente = ?"
            );
            $stmt->execute([
                $id_residente, $id_tipo, $id_personal,
                $fecha_hora, $lugar, $descripcion,
                $accion !== '' ? $accion : null,
                $requiere, $estado, $fecha_cierre, $id
            ]);
    ...
    // Precargar datos para el formulario
    $_POST = $incidente;
    // Formato datetime-local
    $_POST['fecha_hora'] = $incidente['fecha_hora']
        ? date('Y-m-d\TH:i', strtotime($incidente['fecha_hora']))
        : '';
    $_POST['fecha_cierre'] = $incidente['fecha_cierre']
        ? date('Y-m-d\TH:i', strtotime($incidente['fecha_cierre']))
        : '';
    $_POST['requiere_seguimiento'] = $incidente['requiere_seguimiento'] ? '1' : '';
}
...
<div class="row justify-content-center">
    <div class="col-lg-9">
        <h2 class="mb-3">
            <i class="bi bi-pencil"></i> Editar incidente #<?= $id ?>
        </h2>
        ...
        <form method="post" class="card shadow-sm p-4">
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <textarea name="accion_realizada" class="form-control" rows="2"><?= htmlspecialchars($_POST['accion_realizada'] ?? '') ?></textarea>
                </div>
                <div class="col-12">
                    <div class="form-check">
                        <input type="checkbox" name="requiere_seguimiento" class="form-check-input" id="req"
                               <?= !empty($_POST['requiere_seguimiento']) ? 'checked' : '' ?>>
            ...
            <div class="mt-4 d-flex gap-2">
                ...
                <a href="ver.php?id=<?= $id ?>" class="btn btn-outline-info">Ver detalle</a>
